...
def GetDeadlineCommand():
    ...
    try:
        ...
        # however Deadline command may be in the PATH or on OSX
        # it could be in the file /Users/Shared/Thinkbox/DEADLINE_PATH
        pass

    # Determine if the installer is being run on OSX
    if sys.platform == "darwin":
        # On OSX, we look for the DEADLINE_PATH file
...
class SubmitToDeadline_Operator (bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if path != "":
            ...
            # Import is done here because the Deadline repository path
            # needs to be added to the system path (see above)
            # before SubmitBlenderToDeadline.py can be properly referenced in this script.
