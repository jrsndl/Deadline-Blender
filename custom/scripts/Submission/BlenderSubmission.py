...
OCIO_PALLBACK_DEFAULT_PATH = "C:\\_GFX_library\\ocio\\studio_aces-v13_ocio-v21.ocio"
GPU_SCRIPT_DEFAULT_PATH = "C:\\_GFX_library\\blender\\gpurender\\BlenderForceGPUScript2.py"
...
def __main__(*args):
    ...
    blender_file = scriptDialog.AddSelectionControlToGrid( "SceneBox", "FileBrowserControl", "", "Blender Files (*.blend);;All Files (*)", 1, 1, colSpan=2)
    # for autofill
    blender_file.ValueModified.connect(PathChanged)
    ...
    scriptDialog.AddControlToGrid("AdditionalArgumentsLabel", "LabelControl", "Path to BlenderForceGPU Script", 7, 0, "Path to the BlenderForceGPU Script.", False)
    scriptDialog.AddSelectionControlToGrid("AdditionalArgumentsBox", "FileBrowserControl", "", "Python Files (*.py);;All Files (*)", 7, 1, colSpan=2)

    # Version
    scriptDialog.AddControlToGrid("VersionLabel", "LabelControl", "Version", 8, 0, "The version of Blender to render with.", False )
    scriptDialog.AddComboControlToGrid("VersionBox", "ComboControl", BLENDER_VERSIONS[-1], BLENDER_VERSIONS, 8, 1)
    scriptDialog.EndGrid()

    scriptDialog.AddGrid()
    scriptDialog.AddControlToGrid("Separator4", "SeparatorControl", "OCIO Options", 0, 0, colSpan=3)
    scriptDialog.AddSelectionControlToGrid("OCIOEnv", "CheckBoxControl", False, "Use Current OCIO Environment Variable", 1, 0, "If this option is enabled, OCIO env var will be set for the job if exists.")
    scriptDialog.AddSelectionControlToGrid("OCIOFallback", "CheckBoxControl", False,"Use Fallback OCIO Path", 2, 0, "If this option is enabled, fallback OCIO env var will be set for the job if OCIO env doesn't exists.")
    scriptDialog.AddSelectionControlToGrid("OCIOPathBox", "FileBrowserControl", "", "OCIO Files (*.ocio);;All Files (*)", 2, 1, "Fallback OCIO config path", colSpan=2 )
    ...
    settings = ("DepartmentBox", "CategoryBox", "PoolBox", "SecondaryPoolBox",
                ...
                "SubmitSceneBox", "VersionBox",
                "OCIOEnv", "OCIOFallback", "OCIOPathBox")
    ...
    # default
    scriptDialog.SetValue("OCIOPathBox", OCIO_PALLBACK_DEFAULT_PATH)
    ...
    if len(args) > 0:
        ...
        scriptDialog.SetValue("AdditionalArgumentsBox", GPU_SCRIPT_DEFAULT_PATH)
        blender_version = args[5]
        scriptDialog.SetValue("VersionBox", blender_version)
        #scriptDialog.SetValue("DepartmentBox", "DMP")
        scriptDialog.SetValue("PoolBox", "all")
        scriptDialog.SetValue("SecondaryPoolBox", "all")
        scriptDialog.SetValue("GroupBox", "all")
        scriptDialog.SetValue("CommentBox", os.path.basename(args[0]))
...
def PathChanged(*args):
    global scriptDialog
    blend_file = scriptDialog.GetValue("SceneBox")

    scriptDialog.SetValue("NameBox", Path.GetFileNameWithoutExtension( blend_file ))
    scriptDialog.SetValue("CommentBox", os.path.basename(blend_file))
...
def SubmitButtonPressed(*args):
    ...
    # add OCIO env var if exists - BC Fix
    ocio_to_set = None
    ocio_env = os.environ.get("OCIO")
    ocio_use = scriptDialog.GetValue("OCIOEnv")
    ocio_use_fallback = scriptDialog.GetValue("OCIOFallback")
    if ocio_use:
        # set OCIO
        if ocio_env is not None:
            ocio_to_set = ocio_env
        else:
            if ocio_use_fallback:
                ocio_to_set = scriptDialog.GetValue("OCIOPathBox")
    if ocio_to_set is not None:
        writer.WriteLine("EnvironmentKeyValue0=OCIO={}".format(ocio_to_set))
    ...
    writer.WriteLine("AdditionalArguments=%s" % scriptDialog.GetValue("AdditionalArgumentsBox"))

    # Write Version
    writer.WriteLine("Version=%s" % scriptDialog.GetValue("VersionBox"))
    print("Blender version is this: %s" % (scriptDialog.GetValue("VersionBox")))
