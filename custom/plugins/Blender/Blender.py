...
class BlenderPlugin(DeadlinePlugin):
    ...
    def RenderExecutable(self):
        ...
        # Get Version
        blender_version = self.GetPluginInfoEntryWithDefault("Version", "").lower()
        executable = ""
        try:
            executableList = self.GetConfigEntry("Blender_%s_RenderExecutable" % (blender_version))
        except Exception:
    ...
    def RenderArgument(self):
        ...
        # GPU
        gpuscriptpath = self.GetPluginInfoEntryWithDefault("AdditionalArguments", GPU_SCRIPT_DEFAULT_PATH)
        gpuscriptpath = RepositoryUtils.CheckPathMapping(gpuscriptpath)

        renderArgument = " --python-use-system-env -P \"" + gpuscriptpath + "\"  -b \"" + sceneFile + "\""
        ...
        self.LogInfo(f"Blender job parameters:\n{renderArgument}")
